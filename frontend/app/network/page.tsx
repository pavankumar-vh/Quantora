...
export default function NetworkPage() {
    ...
    useEffect(() => {
        ...
        const cachedPositions = positionCache.current;
        ...
        if (simRef.current) {
            simRef.current.nodes().forEach(n => {
                if (n.x != null && n.y != null) {
                    cachedPositions[n.id] = { x: n.x, y: n.y, vx: n.vx || 0, vy: n.vy || 0 };
                }
            });
            simRef.current.stop();
            simRef.current = null;
        ...
        const hasCachedPositions = d3Nodes.some(n => cachedPositions[n.id]);
